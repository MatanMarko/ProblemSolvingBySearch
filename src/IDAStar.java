...
public class IDAStar implements Algorithm {
   ...
   public State solve(String[][] initialState, String[][] goalState, boolean withOpen) {
      int threshold = heuristic(initialState, goalState); // Set the threshold to the heuristic value of the initial state

      while (threshold != Integer.MAX_VALUE) { // While t != "infinity"
         Stack<State> openList = new Stack<>(); // Open list
         Set<State> currentBranch = new HashSet<>(); // The current branch
         int minF = Integer.MAX_VALUE; // Minimum f value
         State start = new State(initialState, 0, "");
         openList.push(start);
         ...
         while (!openList.isEmpty()) {
            ...
            if (f > threshold) { // If f(n) > threshold
               minF = Math.min(minF, f); // Update the minimum f value
               continue;
            }

            if (Arrays.deepEquals(current.getBoard(), goalState)) { // If the current state is the goal state
               current.setNum(nodeNum);
               return current;
            }

            for (State nextState : getNextOptionalStates(current)) {
               if (!currentBranch.contains(nextState)) { // If the neighbor is not in the branch
         ...
         if (minF == Integer.MAX_VALUE) { // If the minimum f value is infinity
